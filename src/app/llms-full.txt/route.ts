...
export function GET() {
  ...
  const body = [
    ...
    `- Overview: ${new URL('/', siteUrl).toString()}`,
    `- Quickstart: ${new URL('/quickstart', siteUrl).toString()}`,
    `- Authentication: ${new URL('/authentication', siteUrl).toString()}`,
    `- Environments: ${new URL('/environments', siteUrl).toString()}`,
    `- Versioning: ${new URL('/versioning', siteUrl).toString()}`,
    `- Request & Response Conventions: ${new URL('/request-response-conventions', siteUrl).toString()}`,
    `- Pagination: ${new URL('/pagination', siteUrl).toString()}`,
    ...
    '',
    '## Golden path URLs',
    `- Restatement drift: ${new URL('/guides/restatement-drift', siteUrl).toString()}`,
    `- Point-in-time research: ${new URL('/guides/point-in-time-research', siteUrl).toString()}`,
    `- Rebuild a historical view: ${new URL('/guides/rebuild-historical-view', siteUrl).toString()}`,
    `- Company resolution and statement retrieval: ${new URL('/guides/company-resolution-and-statement-retrieval', siteUrl).toString()}`,
    '',
    '## Retrieval guidance',
