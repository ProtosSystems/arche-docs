...
function Eyebrow({ tag, label }: { tag?: string; label?: string }) {
  ...
  return (
    <div className="flex items-center gap-x-3">
      {tag && <Tag>{tag}</Tag>}
      {tag && label && (
        <span className="h-0.5 w-0.5 rounded-full bg-[color:var(--docs-border)]" />
      )}
    ...
    >
      {inView && (
        <div className="absolute mt-1 ml-[calc(-1*var(--width))] hidden w-(--width) opacity-0 transition [--width:calc(2.625rem+0.5px+50%-min(50%,calc(var(--container-lg)+(--spacing(8)))))] group-hover:opacity-100 group-focus:opacity-100 md:block lg:z-50 2xl:[--width:--spacing(10)]">
          <div className="group/anchor block h-5 w-5 rounded-lg bg-[color:var(--docs-surface)] ring-1 ring-[color:var(--docs-border)] transition ring-inset hover:bg-[color:var(--docs-hover)] hover:ring-[color:var(--docs-border)]">
            <AnchorIcon className="h-5 w-5 stroke-[color:var(--docs-muted)] transition group-hover/anchor:stroke-[color:var(--docs-text)]" />
          </div>
        </div>
      )}
      {children}
