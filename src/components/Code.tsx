  ...
  return (
    <button
      ...
      className={clsx(
        ...
        copied
          ? 'bg-[color:var(--docs-surface)] ring-1 ring-[color:var(--docs-border)] ring-inset'
    ...
    >
      <span
        aria-hidden={copied}
        className={clsx(
          'pointer-events-none flex items-center gap-0.5 text-[color:var(--docs-muted)] transition duration-300',
          copied && '-translate-y-1.5 opacity-0',
        )}
      >
        <ClipboardIcon className="h-5 w-5 fill-[color:var(--docs-border)] stroke-[color:var(--docs-muted)] transition-colors group-hover/button:stroke-[color:var(--docs-text)]" />
        Copy
      </span>
      <span
        aria-hidden={!copied}
        className={clsx(
          'pointer-events-none absolute inset-0 flex items-center justify-center text-[color:var(--docs-link)] transition duration-300',
          !copied && 'translate-y-1.5 opacity-0',
        )}
      >
        Copied!
...
function CodePanelHeader({ tag, label }: { tag?: string; label?: string }) {
  ...
  return (
    <div className="flex h-9 items-center gap-2 border-y border-t-transparent border-b-[color:var(--code-border)] bg-[color:var(--code-bg)] px-4">
      {tag && (
        <div className="flex">
  ...
  return (
    <div className="group">
      ...
      <div className="relative">
        <pre className="overflow-x-auto p-4 text-xs text-[color:var(--docs-text)]">
          {children}
        </pre>
...
function CodeGroupHeader({
  ...
  return (
    <div className="flex min-h-[calc(--spacing(12)+1px)] flex-wrap items-start gap-x-4 border-b border-[color:var(--code-border)] bg-[color:var(--code-bg)] px-4">
      {title && (
        <h3 className="mr-auto pt-3 text-xs font-semibold text-[color:var(--docs-text)]">
          {title}
        </h3>
      )}
      {hasTabs && (
            ...
            <Tab
              className={clsx(
                ...
                childIndex === selectedIndex
                  ? 'border-[color:var(--docs-link)] text-[color:var(--docs-link)]'
                  : 'border-transparent text-[color:var(--docs-muted)] hover:text-[color:var(--docs-text)]',
              )}
            >
              {getPanelTitle(
...
export function CodeGroup({
  ...
  'code-block my-6 overflow-hidden rounded-2xl bg-[color:var(--code-bg)] shadow-md ring-1 ring-[color:var(--code-border)]'
  let header = (
